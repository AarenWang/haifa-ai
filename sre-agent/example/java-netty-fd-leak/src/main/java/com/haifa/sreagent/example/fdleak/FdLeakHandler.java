...
public final class FdLeakHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
  private final FdLeakController controller;

  public FdLeakHandler(FdLeakController controller) {
  ...
  protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest req) {
    ...
    if ("/status".equals(path)) {
      String json = "{\"fd_running\":" + controller.isRunning()
          + ",\"opened\":" + controller.opened()
          + ",\"max\":" + controller.max()
          + ",\"openPerSec\":" + controller.openPerSec()
          + ",\"dir\":\"" + escape(controller.dir()) + "\""
          + "}";
      RespUtil.sendJson(ctx, req, HttpResponseStatus.OK, json);
      return;
    }

    if ("/fd/start".equals(path) && HttpMethod.POST.equals(method)) {
      int openPerSec = intParam(q.parameters(), "openPerSec", 20);
      int max = intParam(q.parameters(), "max", 256);
      String dir = strParam(q.parameters(), "dir", "/tmp");
      controller.start(openPerSec, max, dir);
      RespUtil.sendJson(ctx, req, HttpResponseStatus.OK, "{\"ok\":true}");
      return;
    }

    if ("/fd/stop".equals(path) && HttpMethod.POST.equals(method)) {
      controller.stop();
      RespUtil.sendJson(ctx, req, HttpResponseStatus.OK, "{\"ok\":true}");
      return;
    }
