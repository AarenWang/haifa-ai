...
## Phase 0 — 现状修复 (1-2 周)
- [DONE] 修复命令注入风险（`pid/service` 参数校验）
- [DONE] 强制只读策略（报告/输出过滤非只读建议）
- [DONE] 统一 Schema 与 prompt 的一致性
- [DONE] 采集层补齐脱敏与审计链路

## Phase 1 — 通用架构落地 (2-4 周)
- [DONE] 引入统一 Orchestrator（state machine / graph）
- [DONE] 建立 Evidence Store（raw/redacted/parsed）
- [DONE] 命令注册中心 + 风险元数据
- [DONE] 适配层落地（LLM Vendor / Agent SDK 可切换）

## Phase 2 — 规则引擎与分类 (2-3 周)
- [DONE] 规则引擎实现基础分类
- [DONE] 支持 CPU / IO / MEM / GC / NET 等主路径
- [DONE] 诊断路径按分类动态路由

## Phase 3 — 扩展与评估 (4-6 周)
- [DONE] 新场景扩展（外部依赖、k8s、容器）
- [DONE] 建立回放与评估框架
- [DONE] 输出稳定性与准确性指标建设
...
## Phase 0 — 现状修复
- [DONE] 增加 `pid/service` 参数校验器与测试
- [DONE] 引入 action policy filter（READ_ONLY 硬约束）
- [DONE] 统一 evidence/report schema 与 prompt 内容
- [DONE] v2/v3 采集链路补齐脱敏与审计哈希
- [DONE] 修复命令白名单缺失（如 `lsof_pid`）与无效命令路径

## Phase 1 — 通用架构落地
- [DONE] 抽象 Orchestrator 接口（State/Node/Edge）
- [DONE] 实现 Evidence Store 及引用机制
- [DONE] 引入 Command Registry + 风险元数据
- [DONE] 增加统一执行器（SSH/MCP 可切换）
- [DONE] 实现 LLM/Agent SDK 适配层与配置切换

## Phase 2 — 规则引擎与分类
- [DONE] 定义 Rule DSL 或规则配置格式
- [DONE] 构建分类器输出标准信号（signals）
- [DONE] 实现动态路由与最小采证策略
- [DONE] 加入反证机制（counter_evidence）

## Phase 3 — 扩展与评估
- [DONE] 扩展 K8s/容器场景探针
- [DONE] 建立离线回放与评估脚本
- [DONE] 增加稳定性监控（schema/timeout/空证据）
- [DONE] 形成回归基线与评分报告

## Phase 4 — 产品化
- [DONE] 对接告警/工单平台输出
- [DONE] 多租户与权限隔离策略
- [DONE] 运营与审计报表（命中率/误报率/执行成本）
